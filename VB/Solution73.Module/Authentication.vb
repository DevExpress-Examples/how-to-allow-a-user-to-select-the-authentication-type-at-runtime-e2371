...
Namespace DevExpress.ExpressApp.Security
	...
	Public Class AuthenticationCombined
		...
		End Sub
		Public Overrides Function Authenticate(ByVal objectSpace As ObjectSpace) As Object
			...
			End If
		End Function
		Private Function AuthenticateStandard(ByVal objectSpace As ObjectSpace) As Object
			...
			End If
			Dim user As IAuthenticationStandardUser = CType(objectSpace.FindObject(UserType, New BinaryOperator("UserName", logonParameters_Renamed.UserName)), IAuthenticationStandardUser)
			...
			Return user
		End Function
		Private Function AuthenticateActiveDirectory(ByVal objectSpace As ObjectSpace) As Object
			Dim userName As String = WindowsIdentity.GetCurrent().Name
			Dim user As IAuthenticationActiveDirectoryUser = CType(objectSpace.FindObject(UserType, New BinaryOperator("UserName", userName)), IAuthenticationActiveDirectoryUser)
			If user Is Nothing Then
				If createUserAutomatically_Renamed Then
					...
					If (Not args.Handled) Then
						user = CType(objectSpace.CreateObject(UserType), IAuthenticationActiveDirectoryUser)
						user.UserName = userName
						If Security IsNot Nothing Then
							'Security.InitializeNewUser(objectSpace, user);
							Dim mi As System.Reflection.MethodInfo = GetType(SecurityBase).GetMethod("InitializeNewUser", System.Reflection.BindingFlags.Instance Or System.Reflection.BindingFlags.NonPublic)
		...
		Public Overrides Function GetBusinessClasses() As IList(Of Type)
			...
			Return result
		End Function
		<Obsolete("Use the 'GetBusinessClasses' method instead.")> _
		Public Overrides Function GetPersistentObjectTypes() As IList(Of Type)
		...
		Public Overrides Property UserType() As Type
			...
			Set(ByVal value As Type)
				...
				If userType_Renamed IsNot Nothing AndAlso (Not GetType(IAuthenticationStandardUser).IsAssignableFrom(userType_Renamed)) AndAlso (Not GetType(IAuthenticationActiveDirectoryUser).IsAssignableFrom(userType_Renamed)) Then
					Throw New ArgumentException(String.Format("AuthenticationCombined does not support the {0} user type." & Constants.vbLf & "A class that implements the IAuthenticationStandardUser interface should be set for the UserType property.", userType_Renamed))
				End If
			End Set
		End Property
